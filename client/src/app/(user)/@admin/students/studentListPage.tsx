import { listStudents } from "../../../api/listStudent";
...
type StudentPageProps = {
  ...
  deleteForm: (id: number, name: string) => JSX.Element;
};
...
const StudentPage = ({
  ...
  return (
    <div className="container flex flex-col mx-auto p-4 w-full">
            ...
            {data.map((student) => (
              <tr key={student.id} className="hover:bg-lamaPurpleLight">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium flex">
                  ...
                  {deleteForm(student.id, student.name)}
                </td>
              </tr>
            ))}
          </tbody>
...
export default async function StudentPageContainer(page: any) {
  ...
  const createForm = <FormModal table="student" type="create" />;
  const deleteForm = (id: number, name: string) => (
    <FormModal table="student" type="delete" id={id} name={name} />
  );
